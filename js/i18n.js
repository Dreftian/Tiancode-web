...
export const I18N = {
  ...
  "page.description": "Tiancode v1.0.51: Estudio de desarrollo local-first para Windows con enjambre de agentes autónomos, Conexiones Gateway (WhatsApp/Telegram/Discord/Slack), resiliencia OpenClaw, memoria Hermes Agent, motor OpenDesign y Sandbox Keep-Alive.",
  "loader.text": "Cargando Tiancode v1.0.51…",
  "nav.showcase": "Studio",
  ...
  "hero.badge": "v1.0.51 · Agente de IA local-first",
  ...
  "hero.ctaDownload": "Descargar Tiancode.exe (v1.0.51)",
  ...
  "preview.sub": "Así se ve Tiancode v1.0.51: Live Preview con Sandbox Keep-Alive, Inspector DOM interactivo, terminal integrada, Conexiones Gateway y skills.",
  ...
  "stats.sub": "Datos reales de la versión 1.0.51.",
  ...
  "cta.note": "Windows 10 y 11 · 64 bits · ~379 MB",
  ...
  "docs.s1p": "Tiancode 1.0.51 se distribuye en dos formatos para Windows 10 y 11 (64 bits):",
  "docs.s1l1": "Instalador NSIS (Tiancode.exe, ~379 MB): instalación clásica con asistente, accesos directos y actualizaciones sencillas.",
  "docs.s1l2": "Versión portable (Tiancode-portable.exe, ~379 MB): sin instalación, se ejecuta desde cualquier carpeta o USB.",
  ...
  "guide.s1d": "Elige el instalador NSIS (~379 MB) o la versión portable (~379 MB) desde la página de descarga. La versión 1.0.51 soporta Windows 10 y 11.",
  ...
  "news.sub": "Tiancode v1.0.51 arregla el botón de mejorar prompt, que fallaba el 100 % de las veces con cualquier modelo y cualquier configuración — y explica por qué no lo detectó nadie: el contexto que le faltaba es una referencia con valor por defecto, así que su tipo dice «never» y el comprobador de tipos no veía nada. Además, los iconos del compositor dejan de pisarse, «Crear respaldo» respalda de verdad, tres desbordes medidos al píxel y un borrado que escala en vez de rendirse. Incluye lo llegado en 1.0.49.",
  "news.v1": "Versión 1.0.51",
  "news.date": "12 de septiembre de 2026",
  "news.chip": "Última versión",
  "news.newTitle": "Novedades principales",
  "news.new1": "El botón de mejorar prompt no funcionó nunca: no era tu modelo, ni la variante «Max», ni el modo 2x — fallaba siempre, con cualquier configuración. La respuesta se entrega como un flujo perezoso y el servidor sólo tira de él después de que el handler haya retornado, momento en el que ya se cerró el ámbito que le inyecta el contexto de la instancia. Moría con «InstanceRef not provided» antes de que un solo byte saliera hacia el proveedor, y el capturador de errores lo disfrazaba de fallo del modelo.",
  "news.new2": "Lo interesante es por qué no lo detectó nadie: ese contexto es una referencia con valor por defecto, así que su tipo dice «never» y el comprobador de tipos no ve nada que falte. Compilaba perfecto y fallaba siempre. Ahora el flujo lleva su propio contexto en vez de pedirlo prestado a la petición, sigue llegando palabra a palabra, y va con un test HTTP contra la ruta real que reproducía el fallo exacto —«\\0unknown», cero texto— antes del arreglo. No había ninguno.",
  "news.new3": "Los iconos del chat se pisaban, y está medido: el grupo izquierdo —agente, modelo, variante— ocupa 315 px rígidos a cualquier ancho y ninguna etiqueta se recorta nunca, así que a partir de 505 px invade el grupo derecho. Tres cerrojos en serie, y el primero es exactamente el mismo bug que las filas de ajustes de esta misma versión: a los botones se les fijó el ancho pero no el «flex», y además cada control va envuelto en un contenedor de tooltip que lo vuelve a clavar. Ahora hay una escalera por ancho del compositor —no de la ventana, que puede ser ancha con el panel estrecho—, las etiquetas se recortan con puntos suspensivos y el botón de enviar sobrevive a cualquier tamaño.",
  "news.new4": "«Crear respaldo» decía una mentira sobre tus propios datos: llamaba a un método que el preload nunca expuso, así que la llamada se tragaba en silencio y te contestaba «no hay datos que respaldar». Ahora respalda de verdad, y el mensaje vacío sólo aparece cuando de verdad no hay nada.",
  "news.new5": "Desbordes medidos: 63 px fuera en «Vetar», 78,6 en «Permitir» y 210,9 en una fila de Conexiones que nadie había reportado. La causa es concreta: el campo de texto lleva «flex: none» y sólo se le sobrescribe el ancho, así que el hueco se dimensiona al tamaño intrínseco del campo y empuja fuera el espacio y el botón entero, que el panel luego recorta. Ya había una clase compartida para «campo + botón», que era justo lo que faltaba. Y la tarjeta de GitHub medía 757 px en un hueco de 608: recortar márgenes no bastaba, así que se parte en dos columnas —el formulario a la izquierda, las capacidades al lado—, sin scroll y con margen a cualquier tamaño.",
  "news.new6": "Borrado: hacer todo lo posible en vez de rendirse. Cuando le das permiso para borrar y Windows se niega, ahora escala — reintenta, quita el atributo de sólo lectura, identifica qué proceso retiene el archivo mediante el Restart Manager (sin administrador y sin herramientas externas) y te dice su nombre y su PID, programa el borrado para el próximo arranque, y sólo si se lo pides explícitamente ofrece cerrar el proceso que lo bloquea. Un borrado parcial ya no aborta al primer archivo bloqueado: los otros nueve mil se borran igual.",
  "news.impTitle": "Mejoras",
  "news.imp1": "Actualizaciones automáticas no destructivas (v1.0.39+): preserva todas tus claves de proveedores, configuraciones y sesiones intactas.",
  "news.imp2": "Las voces se buscaron otra vez, más ancho y midiendo: se descargó la muestra de cada voz y se calculó su frecuencia fundamental en vez de fiarse del nombre. Resultado: las seis etiquetas de sexo del catálogo eran correctas y los rechazos anteriores también («ald» 138,7 Hz, «carlfm» 117,9 Hz y «davefx» 124,6 Hz son voces masculinas; «sharvard» sólo es femenina en su segundo hablante, que es el que ya se usaba). Piper para español está agotado: existen nueve voces y están todas evaluadas.",
  "news.imp3": "Fuera de piper sí apareció una mejor: se sustituye la más floja del catálogo —16 kHz, calidad «low», afinada desde una voz inglesa— por Karen Savage (es-MX), de 22 kHz, con el sexo verificado y una licencia que permite uso comercial. Daniela (Argentina) se mantiene.",
  "news.imp4": "Y lo que no se puede, dicho claro: DeepSeek nunca ha publicado un modelo de voz. Qwen sí, y es Apache-2.0, pero no tiene ninguna voz en español —sus cuatro timbres femeninos son chino, chino, japonés y coreano— y además el motor de Tiancode no puede ejecutarlo: haría falta un segundo runtime de inferencia de más de 600 MB para sonar peor que los modelos de 63-114 MB actuales.",
  "news.imp5": "«Uso de la PC» estrenó la interfaz que le faltaba (1.0.49): permiso del navegador de tres valores, sitios permitidos, cookies del navegador integrado, interruptor maestro guardado fuera del archivo de proyecto y lista de ejecutables denegados. Más la vista de transcripción Normal / Pensando / Detallado en Ajustes → Apariencia.",
  "news.fixTitle": "Correcciones",
  "news.fix1": "Ajustes → General: se borran dos interruptores muertos, «Terminal» y «Navegador interno», cuyos únicos lectores viven en una rama de la interfaz vieja que ya es inalcanzable. Y dos descripciones pasan a decir la verdad: el respaldo automático se aplica en el próximo inicio, y «Mostrar agente» no oculta nada en proyectos que definen sus propios agentes.",
  "news.fix2": "Un modelo que sólo emite razonamiento y ningún texto producía el mismo cuerpo vacío que un fallo, y se anunciaba como «no dijo nada». Ahora es un caso propio.",
  "news.fix3": "Una corrección a la premisa: Tiancode no bloqueaba nada. Se comprobó en tu máquina que con permisos totales Windows sigue rechazando el borrado, porque es un bloqueo obligatorio del sistema — y en tu caso lo sostenía la propia app que lanzaste desde «release\\win-unpacked». Por eso no hay interruptor de «sin restricciones»: habría sido un control conectado a nada. Cuando quien bloquea es Tiancode mismo, ahora te lo dice en vez de fingir.",
  "port.title": "Modo portable",
  "port.sub": "Tiancode donde lo necesites: sin instalación, con tu configuración a cuestas.",
  "port.intro": "La versión portable (Tiancode-portable.exe, ~379 MB) ejecuta la aplicación completa sin instalarla. Es la misma Tiancode, pero todo — el ejecutable, la configuración y los modelos — vive en la carpeta que tú elijas.",
  ...
  "dl.sub": "Tiancode 1.0.51 para Windows 10 y 11 (64 bits). Gratis, sin registro. El instalador y la versión portable son el mismo software: elige el que mejor se adapte a tu flujo.",
  ...
  "dl.news": "¿Qué hay de nuevo en 1.0.51? Lee las novedades.",
  "nav.overview": "Visión General",
  "nav.capabilities": "Capacidades",
  "nav.benchmarks": "Evaluaciones",
  "nav.architecture": "Arquitectura",
  "nav.downloadPill": "Descargar v1.0.51",
  "hero.cosmicLeft": "TIAN",
  "hero.cosmicRight": "CODE",
  "hero.cosmicReplayAria": "Reiniciar animación cósmica de TIANCODE",
  "hero.cosmicBadge": "MODELO DE INTELIGENCIA CÓSMICA · TIANCODE 1.0.51",
  "hero.eyebrowPill": "COMUNICADO DE PRODUCTO",
  "hero.eyebrowMeta": "SEPTIEMBRE 2026 · TIANCODE 1.0.51",
  "hero.title": "Tiancode: Inteligencia Agéntica Autónoma para Desarrolladores",
  "hero.lead": "Un hito en la ingeniería de software soberana y local-first. Impulsado por enjambres multi-agente autónomos, privacidad con cero telemetría, ejecución nativa de Model Hub GGUF, 120+ skills de ingeniería y síntesis de voz neural Kokoro en tiempo real para Windows.",
  ...
  "hero.metaReleaseVal": "Septiembre 2026 · v1.0.51",
  ...
  "dl.installerBtn": "Descargar Tiancode.exe (~379 MB)",
  ...
  "footer.status": "Todos los Sistemas Operativos · Desktop v1.0.51",
  ...
  "page.description": "Tiancode v1.0.51: Sovereign, local-first agentic coding desktop app for Windows with multi-agent swarms, OpenClaw resilience, Hermes Agent memory, Connections Gateway (WhatsApp/Telegram/Discord/Slack), OpenDesign UI engine, and Sandbox Keep-Alive.",
  "loader.text": "Loading Tiancode v1.0.51…",
  "nav.showcase": "Studio",
  ...
  "hero.badge": "v1.0.51 · Local-first AI agent",
  ...
  "hero.ctaDownload": "Download Tiancode.exe (v1.0.51)",
  ...
  "preview.sub": "This is Tiancode v1.0.51: Live Preview with Sandbox Keep-Alive, interactive DOM Inspector, integrated terminal, Connections Gateway, and skills.",
  ...
  "stats.sub": "Real numbers from version 1.0.51.",
  ...
  "cta.note": "Windows 10 and 11 · 64-bit · ~379 MB",
  ...
  "docs.s1p": "Tiancode 1.0.51 ships in two formats for Windows 10 and 11 (64-bit):",
  "docs.s1l1": "NSIS installer (Tiancode.exe, ~379 MB): classic wizard-based install with shortcuts and easy updates.",
  "docs.s1l2": "Portable build (Tiancode-portable.exe, ~379 MB): no install, runs from any folder or USB drive.",
  ...
  "guide.s1d": "Pick the NSIS installer (~379 MB) or the portable build (~379 MB) from the download page. Version 1.0.51 supports Windows 10 and 11.",
  ...
  "news.sub": "Tiancode v1.0.51 fixes the prompt-improve button, which failed 100% of the time on every model and every configuration — and explains why nobody caught it: the context it was missing is a reference with a default value, so its type says “never” and the type checker saw nothing wrong. Plus the composer icons stop overlapping, “Back up now” actually backs up, three overflows measured to the pixel, and a delete path that escalates instead of giving up. Includes what landed in 1.0.49.",
  "news.v1": "Version 1.0.51",
  "news.date": "September 12, 2026",
  "news.chip": "Latest release",
  "news.newTitle": "Major New Features in v1.0.51",
  "news.new1": "The prompt-improve button never worked: it was not your model, nor the “Max” variant, nor 2x mode — it failed always, on every configuration. The response is delivered as a lazy stream and the server only pulls on it after the handler has returned, by which point the scope that injects the instance context has already closed. It died with “InstanceRef not provided” before a single byte left for the provider, and the error catcher disguised it as a model failure.",
  "news.new2": "The interesting part is why nobody caught it: that context is a reference with a default value, so its type says “never” and the type checker sees nothing missing. It compiled perfectly and failed always. The stream now carries its own context instead of borrowing the request's, it still arrives word by word, and it ships with an HTTP test against the real route that reproduced the exact failure —“\\0unknown”, zero text— before the fix. There was none.",
  "news.new3": "The chat icons overlapped, and it is measured: the left group —agent, model, variant— takes a rigid 315 px at every width and no label is ever truncated, so past 505 px it invades the right group. Three locks in series, and the first is exactly the same bug as the settings rows in this same release: the buttons had their width pinned but not their “flex”, and each control is also wrapped in a tooltip container that pins it again. There is now a ladder keyed to the composer's width —not the window's, which can be wide while the panel is narrow—, labels truncate with an ellipsis, and the send button survives any size.",
  "news.new4": "“Back up now” told a lie about your own data: it called a method the preload never exposed, so the call was swallowed in silence and it answered “no data to back up yet”. It now backs up for real, and the empty message only appears when there really is nothing.",
  "news.new5": "Measured overflows: 63 px outside on “Ban”, 78.6 on “Allow”, and 210.9 on a Connections row nobody had reported. The cause is concrete: the text field carries “flex: none” and only its width is overridden, so the slot is sized to the field's intrinsic size and pushes the gap and the whole button outside, which the panel then clips. A shared “field + button” class already existed, and it was exactly what was missing. And the GitHub card measured 757 px in a 608 px budget: trimming margins was not enough, so it splits into two columns —the form on the left, the capabilities beside it—, with no scrolling and margin at any size.",
  "news.new6": "Deletion: do everything possible instead of giving up. When you grant permission to delete and Windows refuses, it now escalates — it retries, clears the read-only attribute, identifies which process is holding the file through the Restart Manager (no administrator, no external tools) and tells you its name and PID, schedules the deletion for the next boot, and only if you explicitly ask does it offer to terminate the blocking process. A partial delete no longer aborts on the first locked file: the other nine thousand are deleted anyway.",
  "news.impTitle": "Improvements in v1.0.51",
  "news.imp1": "Automatic non-destructive updates (v1.0.39+): keeps all your active provider keys, sessions, and configurations intact.",
  "news.imp2": "The voices were searched again, wider and by measuring: each voice's sample was downloaded and its fundamental frequency computed instead of trusting the name. The result: the six sex labels in the catalogue were all correct, and so were the previous rejections (“ald” 138.7 Hz, “carlfm” 117.9 Hz and “davefx” 124.6 Hz are male voices; “sharvard” is only female in its second speaker, which is the one already in use). Spanish piper is exhausted: nine voices exist and all of them have been evaluated.",
  "news.imp3": "Outside piper one better voice did turn up: the weakest in the catalogue —16 kHz, “low” quality, finetuned from an English voice— is replaced by Karen Savage (es-MX), at 22 kHz, with its sex verified and a licence that permits commercial use. Daniela (Argentina) stays.",
  "news.imp4": "And what is not possible, stated plainly: DeepSeek has never published a speech model at all. Qwen has, and it is Apache-2.0, but it has no Spanish voice —its four female timbres are Chinese, Chinese, Japanese and Korean— and Tiancode's engine cannot run it anyway: it would take a second inference runtime of more than 600 MB to sound worse than the current 63-114 MB models.",
  "news.imp5": "“PC usage” got the UI it was missing (1.0.49): a three-value browser permission, allowed sites, integrated-browser cookies, a master switch stored outside the project file, and a denied-executables list. Plus the Normal / Thinking / Detailed transcript view in Settings → Appearance.",
  "news.fixTitle": "Fixes",
  "news.fix1": "Settings → General: two dead switches are removed, “Terminal” and “Internal browser”, whose only readers live in a branch of the old UI that is already unreachable. And two descriptions now tell the truth: the automatic backup applies on the next launch, and “Show agent” hides nothing in projects that define their own agents.",
  "news.fix2": "A model that emits only reasoning and no text produced the same empty body as a failure, and was announced as “it said nothing”. It is now a case of its own.",
  "news.fix3": "A correction to the premise: Tiancode was not locking anything. It was verified on your own machine that with Full Control permissions Windows still refuses the deletion, because it is a mandatory system lock — and in your case the holder was the very app you launched from “release\\win-unpacked”. That is why there is no “unrestricted mode” switch: it would have been a control wired to nothing. When the one holding the lock is Tiancode itself, it now tells you instead of pretending.",
  "port.title": "Portable Mode",
  "port.sub": "Tiancode wherever you need it: no installation, your configuration travels with you.",
  "port.intro": "The portable build (Tiancode-portable.exe, ~379 MB) runs the full application without installing it. It is the same Tiancode, but everything — the executable, the configuration, and the models — lives in a folder of your choice.",
  ...
  "dl.sub": "Tiancode 1.0.51 for Windows 10 and 11 (64-bit). Free, no sign-up required. The installer and the portable build are the same software: pick the one that fits your workflow.",
  ...
  "dl.news": "What is new in 1.0.51? Read the changelog.",
  "nav.overview": "Overview",
  "nav.capabilities": "Capabilities",
  "nav.benchmarks": "Benchmarks",
  "nav.architecture": "Architecture",
  "nav.downloadPill": "Download v1.0.51",
  "hero.cosmicLeft": "TIAN",
  "hero.cosmicRight": "CODE",
  "hero.cosmicReplayAria": "Restart cosmic TIANCODE animation",
  "hero.cosmicBadge": "COSMIC INTELLIGENCE MODEL · TIANCODE 1.0.51",
  "hero.eyebrowPill": "PRODUCT ANNOUNCEMENT",
  "hero.eyebrowMeta": "SEPTEMBER 2026 · TIANCODE 1.0.51",
  "hero.title": "Tiancode: Autonomous Agentic Intelligence for Developers",
  "hero.lead": "A milestone in sovereign, local-first software engineering. Powered by autonomous multi-agent swarms, zero-telemetry privacy, native offline GGUF Model Hub, 120+ engineering skills, and real-time Kokoro neural voice synthesis for Windows.",
  "hero.ctaPortable": "Download Portable (.exe)",
  "hero.cmdCopyTitle": "Click to copy winget command",
  "hero.metaReleaseLabel": "Release",
  "hero.metaReleaseVal": "September 2026 · v1.0.51",
  ...
  "dl.installerBtn": "Download Tiancode.exe (~379 MB)",
  ...
  "footer.status": "All Systems Operational · Desktop v1.0.51",
  "footer.ecosystem": "Ecosistema",
